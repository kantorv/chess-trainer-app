...
| `src/data/library/` | **The Library's shipped collections** (CTA-75) — one `.pgn` file per collection: `WorldCup2023.pgn` (674 games), `Bucharest2023.pgn` (45) and `Morphy.pgn` (211). Dropping a file in adds a collection, named from the file name; its `README.md` says how. The only thing adding content touches. |
| `src/lib/libraryCollections.ts` | **What a Library collection is** (CTA-75) — `LibraryCollection` (id, name, `source`: shipped or uploaded, one PGN chunk per game), the table's columns (`COLLECTION_COLUMNS`) and `collectionRowOf` (a row read off the tags with `readPgnParts`, the length counted off the movetext by `mainlinePlies` — no `chess.js`, so 674 rows cost milliseconds), `sortedRows` / `filteredRows`, `collectionNameOfStem` / `collectionIdOfStem` (a shipped file's name and route segment from its file name) and `readCollectionText` (a file or a paste read as a collection). Pure. |
| `src/lib/shippedCollections.ts` | The shipped collections: a **lazy** `import.meta.glob('../data/library/*.pgn', { query: '?raw' })`, so each file is its own chunk, fetched the first time its collection is asked for; the id and name come from the path without fetching. `shippedCollectionsOf` takes the loaders as a parameter (the drop-in rule's test). |
| `src/lib/libraryCollectionStore.ts` | **The reader's uploaded collections** (CTA-75) — `chessapp.libraryCollections.v1` over `recordStore`, newest first, each one PGN chunk per game: `addCollection` (refused past `MAX_UPLOAD_CHARS`, or reported when the quota refuses the write), `removeCollection`, and a game's **Update** (`replaceCollectionGame`) and **Save as copy** (`insertCollectionGame`, right after the original). Why `localStorage` and what the budget is: its header. Non-throwing throughout. |
...
| `src/lib/pgnText.ts` | Small text rules over PGN that several readers share — `slugify` (a download's or a route's slug), `chapterPrefix` (a repertoire chapter's `"N) "`) and `MAX_UPLOAD_CHARS` (3,000,000 — the most one stored text may be). Pure. |
...
| `src/views/library/` | **The Library** (CTA-75): `LibraryHome.tsx` (`/library` — the collections, shipped then uploaded), `CollectionScreen.tsx` (`/library/<collection>` — the table: sort, filter, pages, download, delete an upload; its state in the URL), `LibraryUpload.tsx` (`/library/new` — a file or a paste becomes a collection), `LibraryGameScreen.tsx` (`/library/<collection>/<game>` — resolves and parses the game) over **`LibraryGameBoard.tsx`** (the v2 analysis board: `useAnalysisSession` + the variations explorer, and the collection's Update / Save as copy), `LibraryMiss.tsx`, and `useLibraryCollections.ts` (the store binding, a shipped file fetched on first use). |
...
```
src/data/library/*.pgn ──lazy glob──▶ shippedCollections ─┐                     ┌─▶ /library/<collection>        (the table)
  (a file = a collection)             (lib/)               ├─▶ LibraryCollection ─┤      sort · filter · pages · download
/library/new (a file or a paste) ──▶ libraryCollectionStore┘   one PGN chunk      └─▶ /library/<collection>/<n>    (the board)
  (a new one-level folder)            (localStorage)            per game               useAnalysisSession + the explorer
```

- **Dropping a `.pgn` into `src/data/library/` is a new collection** — no
  TypeScript, no manifest, no locale key, no route. `lib/shippedCollections.ts`
  globs the folder **lazily** (each file is its own chunk, fetched the first
  time its collection is asked for), names it from the file name
  (`WorldCup2023.pgn` → "World Cup 2023", `collectionNameOfStem`) and addresses
  it by its slug (`/library/worldcup2023`). Three ship: `WorldCup2023` (674
  games), `Bucharest2023` (45) and `Morphy` (211).
- **A game is addressed by its place** — `/library/<collection>/<1-based
  ...
  what a reader says.
- **The table is read without `chess.js`** (`lib/libraryCollections.ts`). A
  row is the game's tags (`readPgnParts`, the reader `parsePgnTree` uses) and
  its length counted off the movetext as text (`mainlinePlies`) — parsing 674
  games to fill one column would be seconds. The columns are the tags the
  shipped files carry: `#`, White, Elo, Black, Elo, Result, Date (its unknown
  ...
  last either way); a words box and a result select **filter**; the rows are
  **paged** (50 / 100 / 250). The sort, filter and page live in the URL (history
...
- **Nothing is written unless the reader asks** — the Analysis Board's changes
  ...
  the board goes on in the copy) or **Discard**. A game of a **shipped**
...
- **Uploads** (`/library/new`): a `.pgn` file or pasted text, one reading for
  both (`readCollectionText`: line endings normalised, cut into games, a text
  with no tag and no SAN move refused), becomes a **new one-level folder** —
  named as typed, else by the `Event` every game shares, else by the file name
  — and the reader lands on its table. An uploaded collection can be deleted
  from its table (asked first).
- **Storage: `localStorage`, deliberately** (`lib/libraryCollectionStore.ts`,
  `chessapp.libraryCollections.v1`). Every other store is synchronous
  `localStorage` read through `useSyncExternalStore`, and a table and a board
  that render on the first frame want the same; IndexedDB would lift the
  ceiling but make every read a promise. The ceiling is the origin's few
  megabytes, shared with every store: one text is refused past
  `MAX_UPLOAD_CHARS` (3,000,000 characters — the World Cup file is ~640,000),
  and a write the quota refuses is reported, never thrown. A collection is
  stored as one chunk per game, so Update and Save as copy touch one element.
