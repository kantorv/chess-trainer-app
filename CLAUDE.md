...
- **Settings are the one thing edited after the fact** (`/repertoires/<id>/settings`):
  ...
  **protected** (`protected`, on by default; CTA-63 — its board's "Update"
  asks instead of writing; copies are never protected). They are one `settings` object normalised field by field, so an
...
- **Changes are kept on the reader's say-so — update, or copy.** Nothing is
  ...
  - **Update repertoire** — `withRepertoireTree` (`lib/savedRepertoires.ts`)
    ...
    shipped or borrowed one — Update does not write: a dialog
    (`RepertoireProtectedDialog.tsx`) offers its settings (to switch
    protection off; leaving the board drops the unsaved changes, and the
    dialog says so) or Save as copy. The screen parses the record it *opened*,
