...
| `src/lib/openingTree.ts` | **A collection's opening tree** (CTA-76) — `openingTreeOf` merges the index rows' `line`s by SAN into one tree in memory (no `chess.js`; each node a game count and its White / draw / Black split, the most played continuation first), `openingNodeAt` walks a line as far as the games follow it (`openingNodeOn`: the whole line, or a node of no games), and `?line=` is the `?at=` encoding (`openingLineParamOf` / `openingLineOfParam`). Which games pass through a node is not stored: they are the rows whose `line` begins with the path — `filteredRows`' `line`. Pure. |
...
| `src/views/library/` | **The Library** (CTA-75): `LibraryHome.tsx` (`/library` — the collections, shipped then uploaded, narrowed by a name box (`?q=`), each row with a download of the whole collection, and an upload's with its delete, in a column beside the row's link), `CollectionScreen.tsx` (`/library/<collection>` — the table: sort, filter, pages, a checkbox per row and the export bar to download the picked games — select-all takes every filtered row — and, in an upload, delete them; its state in the URL) with `CollectionFilters.tsx` (its right-hand panel: player and side at the top, then `OpeningFilterBoard.tsx`, the opening-moves board (CTA-76), then opening, event, dates, result — each only where the games carry it), `LibraryUpload.tsx` (`/library/new` — a file or a paste becomes a collection, or a name alone an empty one; `?into=<id>` adds a file's or a paste's games to the end of an upload), `LibraryGameScreen.tsx` (`/library/<collection>/<game>` — resolves and parses the game) over **`LibraryGameBoard.tsx`** (the v2 analysis board: `useAnalysisSession` + the variations explorer, and the collection's Update / Save as copy), `LibraryMiss.tsx`, `useLibraryCollections.ts` (a collection's three parts — summary, rows, games — each read only when a screen needs it) and `indexCollection.ts` (an upload's index pass in the worker, with progress and cancel). |
...
- **Filtering by opening moves** (CTA-76). Under the player and side filters
  ...
  by SAN in memory from **the games the other filters leave** — filter by a
  player and side, and the board shows that player's openings; rebuilding
  it takes under 10 ms for 10,000 games. From the position on it the
  ...
  flip sit over it. The line is `?line=e4,c5,Nf3` (the `?at=` encoding),
  cut back only where no game of the **whole** collection follows it (a
  stale link); where only the other filters leave no game on it, it is kept
  as written, the table is empty and the board says so. Clear takes it off
  with the rest. An index from before the
...
- **Picking and downloading a batch.** Every row carries a checkbox, and the
  ...
  **Saved analyses**: one new top-level folder named for the collection, the
  count and the filters that are on — `World Cup 2023 — 12 games (Carlsen,
  white, B90, 1.e4 c5)` (`batchFolderNameOf` / `activeFilterSummary` in
  `lib/libraryCollections.ts`, kept within the folder name's 100 characters,
  the filter summary cut first) — and every picked game saved into it as its
  own analysis, in collection order, all or nothing: the Load tab's split
  over games already read (`batchAnalysesOf`: each game's PGN as stored, not
  re-parsed — the index already parsed it, and a re-write through a tree
  would cost over a minute on a 7,818-game pick), the folder taken back out
  if the records cannot be written. A game the index marks **unreadable is
  left out**, and the notice says how many. The button spins while it works;
  a snackbar says how many went where, with a link to
  `/tools/analysis/saved?folder=<id>`; the picks stay. Shipped and uploaded
  collections alike.
  The rows are **paged**
...
- **Uploads** (`/library/new`): a `.pgn` file or pasted text, one reading for
  ...
  (asked first).
- **Custom collections** (CTA-77): the same screen makes an **empty**
  collection from the name alone ("New collection" when none is typed), and
  an upload's table has **Add games** — `/library/new?into=<id>`, the upload
  screen again (one reading, the worker's check, the progress bar and Cancel),
  whose games are added at the end of that collection
  (`appendCollectionGames`, games and rows in one transaction) before the
  reader goes back to its table. A shipped collection takes none (the miss).
