...
| `src/lib/savedRepertoires.ts` + `savedRepertoireStore.ts` | **The reader's repertoires** (CTA-61) — one record per brought-in file, holding the whole text (the 310-line Alapin example is one row), a name (typed, else the tags' `StudyName` / `Event`), the `previewFen` where every line still agrees, and a `folderId` that is always `null` until folders arrive, and its `settings` (`lib/repertoireSettings.ts` — description and main color today, read back field by field so a new option is never a version bump; that file's header is the recipe for adding one), edited in place by `updateRepertoireSettings`. `checkRepertoirePgn` is the one check a file and a paste share (the uploads' size/emptiness rules, then every line parsed as a tree); `savedRepertoireOf` the one constructor, which normalises line endings and never reads a file name — so a file and its pasted text are the same record. `repertoireLinesOf` lists the lines under their `"N) "` chapters off the tags alone, without parsing a move. The store is `chessapp.savedRepertoires.v1` over `recordStore`. Non-throwing throughout. |
...
| `src/views/dev/` | **The Development section** (CTA-60) — dev-only, gated on `import.meta.env.DEV` in `navFolders()` / `navItems()` / `App.tsx`, so none of it reaches the deployed build. `core/` is the **unified board core** specified in [`.claude/rules/chessboard-v2.md`](.claude/rules/chessboard-v2.md): the base hook (`useBoardCore.ts` — the `GameTree` as the one game shape, node navigation, the rules oracle, promotion, orientation), the capability modules a board composes rather than is flagged by (`useEngineModule.ts`, whose optional `onBestMove` is the entire Play/Analysis difference; `useOpeningBookModule.ts`; `useAutosave.ts`; `devStores.ts`, the dev-prefixed `localStorage` keys over the shipped `recordStore` factory and normalisers), and the composition layer that had no owner before — `BoardShell.tsx` (over the shared `EngineBoardSquare`, never a second copy of the `calc()`) and `BoardPanel.tsx`, the one panel skeleton and the one pinned best-variations block, plus `TreeMoveList.tsx`, the merged move list's ply↔node seam. Beside them the five derived boards — `analysis/`, `play/` (whose `usePlayBoard` and `PlayBoardScreen` `masked/` reuses verbatim, adding only the mask), `masked/`, `openings/`, `repertoire/` — and `devNav.ts`. The five shipped board screens are untouched by all of it. **`core/` alone ships** since CTA-61, imported by the Repertoires board; the five derived boards, `devNav.ts` and `devStores.ts` stay behind the gate. |
| `src/views/repertoires/` | **The Repertoires section** (CTA-61) — the reader's own repertoires. `Repertoires.tsx` is the list (`/repertoires`, over the saved-list machinery; a card previews where the lines first branch), `RepertoireUpload.tsx` brings one in (`/repertoires/new`, a `.pgn` file or pasted text through **one** function), `RepertoireBoard.tsx` reads one (`/repertoires/<id>`) — the first shipped screen composed from the v2 core, with `RepertoireLines.tsx` as its Lines tab — and `RepertoireSettingsScreen.tsx` edits one (`/repertoires/<id>/settings`: a list of sections from `RepertoireSettingsSections.tsx` over one draft, written on Save). `useSavedRepertoires.ts` is the store binding; `repertoireTestKit.tsx` the tests' shared mount. |
...
A repertoire the reader brings in at `/repertoires/new` is kept **whole**, as
the text of the file (`lib/savedRepertoires.ts`), and read on one v2 board at
`/repertoires/<id>`. It is the Saved openings section again for the list, and
Repertoire v2 again for the board. Only the differences are written out here:

- **A file and a paste are one record.** Both go through `checkRepertoirePgn`
  and `savedRepertoireOf`. Line endings are normalised and a file name is never
  read, so the two routes cannot drift apart. `RepertoireUpload.test.tsx`
  asserts it through the screen.
- **The lines are read off the tags, and a line's moves only when it is picked.**
  The Lines tab lists 310 lines without a parse. Picking one parses its tree
  and hands it to the base's `loadTree`: same board, same engine worker, no
  route change.
