...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **Analysis Board** (Analysis v2, the reference — shipped, CTA-73) | `/tools/analysis` | `?fen=`, `?game=`+`?move=`, `?analysis=`, `?at=` | ✅ switch, **no reply** | header line only | ❌ **explicit save** — Save opens the changes strip (Update / Save as copy / Discard) over a record, or a name-and-folder dialog; the real store, nested folders | Moves · Map · Load · Export · Engine | name + opening + Save + Play-from-here + saved list + switch | comment block, the changes strip, the next-moves bar | next-move arrows (a switch, on) | **`useVariationsExplorer`** — Moves, Map, comment block, next-moves bar, arrows; editing on, `playChances: false` |
