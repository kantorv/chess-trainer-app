...
| `/library` · `/library/<collection>` · `/library/new` | [`views/library/LibraryHome.tsx`](../../src/views/library/LibraryHome.tsx), [`CollectionScreen.tsx`](../../src/views/library/CollectionScreen.tsx), [`LibraryUpload.tsx`](../../src/views/library/LibraryUpload.tsx) | (composed) | **The Library** (CTA-75) — no game board: the collections (the shipped ones wired by `node scripts/wirepgn.js` and listed off `src/data/library/manifest.json` with no fetch, then the reader's uploads in IndexedDB), a collection's games as a **sortable, filterable table** read off its **index** (`lib/collectionIndex.ts` — built once: tags plus a `chess.js` pass, an unreadable game marked; MUI `Table` with `TableSortLabel`, a words box, and a right-hand panel of filters — player and side at the top, then a small **opening-moves board** (`OpeningFilterBoard.tsx`, CTA-76, `options.id` `library-filter-board`, pinned LTR): the opening tree of the games the other filters leave (`lib/openingTree.ts`, merged from the index's `line` column) drawn as `nextMoveArrowsOf` arrows and an explorer list with game counts, only moves those games played taken, the line in `?line=` narrowing the table on top of every other filter — and under the board opening or ECO, event, a date range, result, each shown only where the games carry that field; a checkbox per row and the saved lists' export bar, whose select-all takes every filtered row on every page and downloads them as one `.pgn` (and, in an upload, deletes them, asked first — an upload itself is deleted from its row on `/library`), and beside it **Analyse** (CTA-77), which saves the picks into a new Saved analyses folder named for the collection, the count and the active filters, one analysis per game, with a snackbar linking to it; paged; the state in the URL), and a `.pgn` file or a paste checked game by game in a Web Worker, under a progress bar, before it becomes a new collection. Sized for 5,000–10,000 games. See the root `CLAUDE.md` |
| `/library/<collection>/<game>` | [`views/library/LibraryGameBoard.tsx`](../../src/views/library/LibraryGameBoard.tsx), behind [`LibraryGameScreen.tsx`](../../src/views/library/LibraryGameScreen.tsx) | (composed) | **A v2 screen from the start** (CTA-75) — a collection's game (its 1-based number) on a full analysis board: the Analysis Board's session (`useAnalysisSession`: core, engine, Play) and the variations explorer, tabs Moves (with the next-move arrows' switch) · Map · Info · Export (which also opens the game on the Analysis Board, `?game=library/<collection>/<n>`, CTA-77) · Engine, previous / next game in the header. **Saved explicitly**: an uploaded collection's game is updated in place or copied right after itself; a shipped one is read-only, and its copy goes to Saved analyses. Writes `?at=` back. `options.id` is `library-game` |
