...
| `/repertoires/<id>/play` | [`views/repertoires/RepertoirePlay.tsx`](../../src/views/repertoires/RepertoirePlay.tsx) | (composed) | **Drill a repertoire against a trainer** (CTA-63) — the second shipped board on the v2 core, with `useTrainerModule` ([`chessboard-v2.md`](./chessboard-v2.md) §2.5) in place of the engine: a scripted opponent that answers only from the file, uniformly at random, and only after the reader has moved (stepping back never triggers it). The reader picks a side (default: the main color; the board faces it); a move the file does not have is added under the node on screen and tinted in the move list; past a line's end the reader moves both colours. Two tabs, Moves · Settings (the side and the arrows). Next-move arrows behind a Settings switch, off by default (mainline and side lines in two colours, `nextMoveArrowsOf`). No engine, no autosave — the header's download writes the extended tree as PGN, and the stored record is never touched. `options.id` is `repertoire-play` |
