...
| --- | --- | --- | --- | --- | --- | --- |
| **Analysis Board** `/tools/analysis` | `useAnalysisSession` (+ the saved record: `useAnalysisBoard`) | Play (off at start) | — | explicit: Save → changes strip or name-and-folder dialog | Moves · Map · Load · Position · Export · Engine | explorer, editing on, *Play chances…* off, `addedIds` |
