...
- **An analysis board moves a piece only when the reader presses Play**
  (CTA-73). Its `onBestMove` plays a finished search's best move at the node
  on screen only while the header's Play toggle is on (and the engine is), so
  the engine goes on playing both sides until paused; off, a `bestmove` is
  ignored. Play is disabled while the engine is off, and pauses itself when
  the position is over. The play screen's reply is different in kind — it
