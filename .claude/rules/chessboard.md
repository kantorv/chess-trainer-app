...
| `/repertoires/<id>` | [`views/repertoires/RepertoireBoard.tsx`](../../src/views/repertoires/RepertoireBoard.tsx) | (composed) | **The first shipped board built from the v2 core** ([`chessboard-v2.md`](./chessboard-v2.md)) — one of the reader's own repertoires (CTA-61) — **one game**, a mainline with its side lines (Moves · Engine; a text of several games is merged into one tree or split into many on the way in). Its tree is parsed behind a `setTimeout(0)`, because the 9,146-node example takes ~1s. `/repertoires` lists them over the saved-list machinery (preview board `repertoires-preview-<id>`), filed into one level of folders (`?folder=<id>`; a split lands in a folder of its own); `/repertoires/new` brings one in from a file or a paste; `/repertoires/<id>/settings` edits its title, description and main color — the board opens facing that color (`useBoardCore`'s `orientation`, read once), and so does the preview card |
| `/repertoires/<id>/play` | [`views/repertoires/RepertoirePlay.tsx`](../../src/views/repertoires/RepertoirePlay.tsx) | (composed) | **Drill a repertoire against a trainer** (CTA-63) — the second shipped board on the v2 core, with `useTrainerModule` ([`chessboard-v2.md`](./chessboard-v2.md) §2.5) as the opponent: a scripted opponent that answers only from the file, uniformly at random, and only after the reader has moved (stepping back never triggers it). The reader picks a side (default: the main color; the board faces it); a move the file does not have is added under the node on screen and tinted in the move list; past a line's end the reader moves both colours. Tabs Moves · Settings · Engine, plus **Score** after Moves while **game mode** is on (each of the reader's moves inside the repertoire judged before it is made — a wrong one taken back, one verdict per position, a session tally): Settings holds the side, game mode, the arrows and the engine's switch; the Engine tab is disabled while the engine is off, which it is **by default**; on, it fills the pinned best-variations block and the eval bar and never moves a piece. Next-move arrows behind a Settings switch, off by default (mainline and side lines in two colours, `nextMoveArrowsOf`). No autosave — the header's download writes the extended tree as PGN, and the stored record is never touched. `options.id` is `repertoire-play` |
