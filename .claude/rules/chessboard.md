...
| `/library/*` | [`views/pgn/UserPgnsSection.tsx`](../../src/views/pgn/UserPgnsSection.tsx) | (composed) | The **Library** section (was "User PGNs" at `/pgn/*` — old URLs redirect): the same two screens again, over a library whose items are whole **games** out of the project's `.pgn` files. A card previews the game's starting position; the detail screen is [`views/library/LibraryGameDetail.tsx`](../../src/views/library/LibraryGameDetail.tsx), which replays it over the shared `MoveList` / `BoardControls` / `useGameNavigation` and hands it on with `?game=`. A **repertoire** line instead opens in [`views/library/LibraryVariationDetail.tsx`](../../src/views/library/LibraryVariationDetail.tsx) — `parsePgnTree` + the shared `VariationTree`, since a repertoire's `( )` side lines are the content |
